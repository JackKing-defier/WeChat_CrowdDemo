...
/***/ function (module, exports, __webpack_require__) {
      ...
      Page({
        ...
        onLoad: function onLoad(options) {
          ...
          wx.getLocation({
            ...
            success: function (res) {
              //console.log(res)
              that.ajax2(res.latitude, res.longitude)
            },
            fail: function (res) {
            }
          })
          ...
          try {
            var value = wx.getStorageSync('cjdg')
            if (value) {
              console.log(value)
              this.setData({
                cityArr: value,
        ...
        searchSubmit: function (e) {
          var that = this
          var cityObj = this.data.cityList
          var citySearch = []
          ...
          for (var i = 0; i < cityObj.length; i++) {
            for (var j = 0; j < cityObj[i].cityInfo.length; j++) {
              if (cityObj[i].cityInfo[j].city.indexOf(e.detail.value.keywords) > -1 || cityObj[i].cityInfo[j].city.indexOf(e.detail.value) > -1) {
                citySearch.push(cityObj[i].cityInfo[j].city)
              }
            }
          }
          ...
          // console.log(that.data.citySearch)
          // console.log('--------')
        },
        // 搜索 结束
        ...
        wxSortPickerViewItemTap: function wxSortPickerViewItemTap(e) {
          ...
          try {
            console.log(cityArr);
            wx.setStorageSync('cjdg', cityArr)
            wx.setStorageSync('xzcs', city + '市')
            wx.setStorageSync('rcityname', city)
            wx.navigateBack({
              delta: 1
            })
          } catch (e) {
        ...
        ajax2: function (latitude, longitude) {
          var that = this
          var dizhi = ''
          wx.request({
            url: app.globalData.domain + '/api/mation/getgpsear',
            method: 'POST',
            data: {
              latitude: latitude,
              longitude: longitude
